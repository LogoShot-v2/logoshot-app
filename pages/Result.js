    ...
    {datesBactches.data.length !== 0 ?(
      ...
      <Scroll>
        <View style={styles.searchResults}>
          {datesBactches?<>
            <Text style={{ fontWeight: "bold", fontSize: 18, marginVertical: 10, marginLeft: 15 }}>{datesBactches? datesBactches.data.length: '' } result(s) found.</Text>
              ...
              <View style={{ ...styles.searchResultsBox, borderRightWidth: 1 }}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    navigate("ResultDetail", {trademarkDetail: datesBactches.data[2*idx]});
                  }}
                >
                  <Image source={{
                    uri:
                      "http://140.112.106.88:8082/"
                      + datesBactches.data[2*idx]["_source"]["tmark-image-url_1"]

                  }}
                    style={styles.searchResultsImage} />
                  <Text style={styles.searchResultsText}>{datesBactches.data[2*idx]["_source"]["tmark-name"].replace(/\s*/g,"")}</Text>
                </TouchableOpacity>
              </View>
              <View style={styles.searchResultsBox}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                     ...
                     navigate("ResultDetail", {trademarkDetail: datesBactches.data[2*idx+1]
                     });
                  }}
                >
                  <Image source={{  
                    uri:
                      "http://140.112.106.88:8082/"
                      + datesBactches.data[2*idx+1]["_source"]["tmark-image-url_1"]
                  }} style={styles.searchResultsImage} />
                  <Text style={styles.searchResultsText}>{  datesBactches.data[2*idx+1]["_source"]["tmark-name"].replace(/\s*/g,"")}</Text>
                </TouchableOpacity>
              </View>
            </View>
          ))}
